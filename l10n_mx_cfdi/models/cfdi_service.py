...
class CFDIService(models.Model):
    ...
    def create_cfdi(self, cfdi, issuer=None):
        ...
        # recover() shapes such as SW Sapien's raw dict response). Some PACs
        # (Finkok, Comercio Digital, MYSuite, SW Sapien) only accept XML on
        # stamp/issue and the PDF is generated locally by the module anyway.
        accept = Accept.XML_PDF if provider.accept_pdf else Accept.XML
        try:
