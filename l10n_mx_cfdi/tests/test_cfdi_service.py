...
class TestCFDIService(TransactionCase):
    ...
    def test_create_cfdi_uses_issue_when_supported(self):
        ...
        # Finkok does not accept PDF on stamp/issue: request XML only.
        self.assertEqual(mock_pac.issue.call_args.kwargs.get("accept"), Accept.XML)

    def test_create_cfdi_requests_pdf_when_pac_supports_it(self):
        self.cfdi_service.provider = "diverza"
        self.cfdi_service.pac_rfc = "PAF001"
        self.cfdi_service.pac_client_id = "client-1"
        self.cfdi_service.password = "token"
        mock_pac = MagicMock()
        mock_pac.issue.return_value = Document(
            document_id="doc-2",
            xml=(
        ...
        with patch.object(type(self.cfdi_service), "_get_pac", return_value=mock_pac):
            result = self.cfdi_service.create_cfdi(cfdi, issuer=self.issuer)
        self.assertEqual(result["status"], "published")
        # Diverza accepts a PDF alongside the stamped XML.
