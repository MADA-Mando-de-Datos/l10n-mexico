...
class TestCfdiParserEdgeCases(VendorBillTestCommon):
    ...
    def test_foreign_partner_skips_vat_and_mx_country(self):
        # Exercise the create path with a unique emisor so no demo partner matches.
        self.env["res.partner"].search([("vat", "=", RFC_FOREIGN)]).write(
            {"vat": False}
        )
        move = self._create_bill(
            self._cfdi_xml(
                ...
                emisor_nombre="EDGE TEST PROVEEDOR EXTRANJERO",
                folio="F1",
            )
        )
        self.assertTrue(move)
        self.assertEqual(move.partner_id.name, "EDGE TEST PROVEEDOR EXTRANJERO")
    ...
    def test_public_partner_skips_vat_keeps_mx(self):
        # Temporarily clear the generic RFC so the emisor creates a fresh partner.
        self.env["res.partner"].search([("vat", "=", RFC_PUBLIC)]).write(
            {"vat": False}
        )
        move = self._create_bill(
            self._cfdi_xml(
                ...
                emisor_nombre="EDGE TEST PUBLICO EN GENERAL",
                folio="P1",
            )
        )
        self.assertTrue(move)
        self.assertEqual(move.partner_id.name, "EDGE TEST PUBLICO EN GENERAL")
        self.assertFalse(move.partner_id.vat)
